...
所有前端开发都在 `./app/frontend/` 文件夹下进行。最终通过 `npm run build` 生成静态ES6前端资源，后端在开发模式和打包时都能自动路由到build后的资源路径。
